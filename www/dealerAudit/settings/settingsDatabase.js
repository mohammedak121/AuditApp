 ...
 */
angular.module('dealerAudit.settingsDbModuleDB', []).factory('settingsDbfctry', ['$cordovaSQLite', '$q', '$timeout', 'dealerAudit_ConstantsConst', '$rootScope', function($cordovaSQLite, $q, $timeout, dealerAudit_ConstantsConst, $rootScope) {
	...
	try {
		...
		return {
			...
			/**
			 * @function getSelectedLanguageForUser
			 * @description function for getting set Language   from database for thar user.
			 */
			getSelectedLanguage: function() {
				try {
					console.log('Entered into funtion getSelectedLanguage');
					var data = '';
					var query = "SELECT Language FROM Language  WHERE TTS_Name = '" + dealerAudit_ConstantsConst.TTS_Name + "'";
					console.log('query::getSelectedLanguage' + query);
					return $q.when($cordovaSQLite.execute(db, query).then(function(res) {
						if(res.rows.length > 0) {
							data = res.rows.item(0).Language;
						} else {
							console.log("No results found in getSelectedLanguage");
						}
						return data;
					}));
				} catch(e) {
					console.log('Error in getSelectedLanguage:: ' + e);
				}
				console.log("Exited getSelectedLanguage");

			},

			/**
			 * @function getSelectedCountry
			 * @description function for getting set Country   from database for thar user.
			 */
			getSelectedCountry: function() {
				try {
					console.log('Entered into funtion getSelectedCountry');
					var data = '';
					var query = "SELECT Country FROM Country  WHERE TTS_Name = '" + dealerAudit_ConstantsConst.TTS_Name + "'";
					console.log('query::getSelectedCountry' + query);
					return $q.when($cordovaSQLite.execute(db, query).then(function(res) {
						if(res.rows.length > 0) {
							data = res.rows.item(0).Country;
						} else {
							console.log("No results found in getSelectedCountry");
						}
						return data;
					}));
				} catch(e) {
					console.log('Error in getSelectedCountry:: ' + e);
				}
				console.log("Exited getSelectedCountry");
			 ...
			 * @function SetCountry
			 * @param {String} TTS_Name Holds TTS_Name
			 * @param {Intiger} Country Holds the selected Country
			 * @desc function which inserts TTS_Name and Country to  Country  table
			 */

			SetCountry: function(TTS_Name, Country) {
				try {
					console.log('Entered into function SetCountry');
					var executeQuerry;
					//var dbArray = [];
					//console.log('Entered into function SetCountry');
					var insertQuerry = "INSERT OR REPLACE INTO Country(Country,TTS_Name)";
					insertQuerry += " VALUES ('" + Country + "','" + TTS_Name + "') ";
					console.log("Insert query" + insertQuerry);
					executeQuerry = insertQuerry;
					$cordovaSQLite.execute(db, executeQuerry).then(function(response) {
						console.log('response ==>' + JSON.stringify(response));
					}, function(e) {
						console.log('error in set Country ===>' + JSON.stringify(e));
					});

					console.log("executeQuerry::" + executeQuerry);
				} catch(e) {
					console.log('Error in function SetCountry' + e);
				}

				console.log('Exit from function SetCountry');
			 ...
			 * @function getStatus
			 * @param {String} callback Holds fucntion to call back after executing getPin function
			 * @description function which gets the status of the current user to status 1 and remaining users to status 0 in User table
			 */
			getStatus: function(Email) {
				...
				try {
					...
					var query = "SELECT TTS_Name FROM User_Master WHERE Status = '" + Status + "' ";
					console.log('query status::' + query);
					return $q.when($cordovaSQLite.execute(db, query).then(function(res) {
						if(res.rows.length > 0) {
							for(var i = 0; i < res.rows.length; i++) {
								UserArray.push(res.rows.item(i));
							}
						} else {
							console.log("No results found");
						}
						query = "SELECT UserID FROM User_Master WHERE TTS_Name = '" + Email + "' ";
						console.log('query EmailAddress::' + query);
						return $q.when($cordovaSQLite.execute(db, query).then(function(res) {
							console.log('controll inside get UserID in getStatus');
							if(res.rows.length > 0) {
								dealerAudit_ConstantsConst.userId = res.rows.item(0).UserID;
							} else {
								console.log("No results found");
							}
							console.log('User::' + JSON.stringify(UserArray));
							console.log('UserID in getStatus---->' + dealerAudit_ConstantsConst.userId);
							return UserArray;
						}));
					}));
					...
					console.log("Error in getStatus::" + e);
			...
			/**
			 * @function getLanguageCountryInfo
			 * @description Get the languages and the countries the TTS belongs to.
			 */
			getLanguageCountryInfo: function() {
				var countriesArray = [];
				var languagesArray = [];
				var countriesLanguagesArray = [];
				var groupedCountryArray = [];

				var selectQuery = "SELECT * from template_master";

				return $q.when($cordovaSQLite.execute(db, selectQuery).then(function(res) {
					if(res.rows.length > 0) {
						for(var i = 0; i < res.rows.length; i++) {

							// console.log("Country " + i + "Name : " + res.rows.item(i).country_name);
							// console.log("Language " + i + "Name : " + res.rows.item(i).language_name);

							// countriesArray.push(res.rows.item(i).country_name);
							// languagesArray.push(res.rows.item(i).language_name);

							countriesLanguagesArray.push(res.rows.item(i));
						}

						// if(countriesArray.length > 0 && languagesArray.length > 0) {
						// 	countriesLanguagesArray.push(countriesArray);
						// 	countriesLanguagesArray.push(languagesArray);
						// }

						// console.log("Countries languages array : " + countriesLanguagesArray);
						// console.log("Countries languages array : " + JSON.stringify(countriesLanguagesArray));

						groupedCountryArray = _.groupByMultipleFields(countriesLanguagesArray, ['country_name']);

						console.log("Grouped country array : " + groupedCountryArray);
						console.log("Grouped country array stringified : " + JSON.stringify(groupedCountryArray));

						return groupedCountryArray;
					} else {
						return groupedCountryArray;
					}
				}))
			 ...
			 * @description Set the languages and the countries the TTS belongs to the local database.
			 */
			setLanguageCountry: function(language, country) {

				console.log("Entered into the function setLanguageCountry");

				var insertQuery = "INSERT OR REPLACE INTO tts_country_language( language_name ,country_name ) VALUES";

				insertQuery += "('" + language + "','" + country + "')";

				$cordovaSQLite.execute(db, insertQuery);
			},

			/**
			 * @function getTemplateDetails
			 * @description Based on the country and the language get the template details.
			 */
			getTemplateDetails: function(country, language) {

				var templateId = "";
				console.log("Entered into the function getTemplateDetails");


				console.log("Country " + country + " Language " + language);
				var selectQuery = "SELECT template_id FROM template_master WHERE country_name = '" + country + "' AND language_name = '" + language + "'";

				return $q.when($cordovaSQLite.execute(db, selectQuery).then(function(res) {
					if(res.rows.length > 0) {
						templateId = res.rows.item(0).template_id;
					}
