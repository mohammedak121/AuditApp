 ...
 */
settingsModule.controller('SettingsCtrl', function($scope, $location, $cordovaSQLite, $ionicPopup, dealerAudit_ConstantsConst, dealerAudit_AssetsConst, settingsDbfctry, $ionicPopup, $filter, $timeout, $rootScope, focus, logsFctry, $cordovaNetwork, syncModuleFactory) {
	...
	$scope.$on('$ionicView.beforeEnter', function() {
		...
		// Get the country and the languages.
		settingsDbfctry.getLanguageCountryInfo().then(function(response) {
			if(typeof(response) != "undefined" || response != null) {
				console.log("Countries languages array from controller " + response)
				console.log("Countries languages array from controller stringified" + JSON.stringify(response));

				$scope.languageCountryResponse = response;

				var keys = Object.keys($scope.languageCountryResponse);
				...
				// 
				// 		// First array is a country array.
				// 		if(i == 0) {
				// 			// console.log("Response country name " + response[j].country_name);
				// 			console.log("Response country name " + response[i][j]);
				// 			$scope.countryOptions.push({
				// 				'country': response[i][j],
				//         'lang': response[i][j],
				// 			});
				// 		}
				// 
				// 		// Second one is a language array.
				// 		if(i == 1) {
				// 			$scope.languageOptions.push({
				// 				'text': response[i][j]
				// 			});
				// 		}
				// 	}
		...
		//get the selected language from db and populate before entering into the page
		settingsDbfctry.getSelectedLanguage().then(function(arrayMe) {
			try {
				console.log('Entered into getSelectedLanguage::' + arrayMe);
				dealerAudit_ConstantsConst.currentLanguage = arrayMe;
				console.log('Entered into getSelectedLanguage in before enter::' + dealerAudit_ConstantsConst.currentLanguage);
				$scope.languageSelected.text = dealerAudit_ConstantsConst.currentLanguage;
			} catch(e) {
				console.log('getSelectedLanguage Error:::' + e);
			}
			console.log('Exit form getSelectedLanguage');
		});

		//get the selected Country from db and populate before entering into the page
		settingsDbfctry.getSelectedCountry().then(function(data) {
			try {
				console.log('Entered into getSelectedCountry::' + data);
				dealerAudit_ConstantsConst.currentCountry = data;
				console.log('Entered into getSelectedCountry in before enter::' + dealerAudit_ConstantsConst.currentCountry);
				$scope.countrySelected.text = dealerAudit_ConstantsConst.currentCountry;
			} catch(e) {
				console.log('getSelectedCountry Error:::' + e);
			}
			console.log('Exit form getSelectedCountry');
		});
	});
	 ...
	 * @description This module includes functionalities for initialising logsModule.
	 */
	...
	$scope.setLanguage = function(data) {
		...
		if(typeof(data.text) != "undefined" || data.text != null || data.text != "") {
			...
			$scope.languageSelected.text = data.text;
		}
	};
	...
	$scope.setCountry = function(selectedCountry) {
		if(selectedCountry.text != null || typeof(selectedCountry.text) != "undefined") {
			console.log("Value text", selectedCountry.text);
			settingsDbfctry.SetCountry($scope.settings.TTS_Name, selectedCountry.text);
			$scope.countrySelected.text = selectedCountry.text;

			// Based on the country selected populate the dropdown value.
			// $scope.languageCountryResponse[selectedCountry.text]

			for(country in $scope.languageCountryResponse) {

				console.log("Value : " + country);
				if(selectedCountry.text == country) {
	...
	$scope.SetPin = function(TTS_Name, User_Pin) {
		...
		try {
			...
			// Save the selected language and country and get the questions based on the template id.			
			console.log("Selected language from dropdown : " + $scope.languageSelected.text);
			console.log("Selected country from dropdown : " + $scope.countrySelected.text);
			settingsDbfctry.setLanguageCountry($scope.languageSelected.text, $scope.countrySelected.text);

			// Get the template id based on the country and the language selected from template_master 
			// Download the questions related to the template.
			settingsDbfctry.getTemplateDetails($scope.countrySelected.text, $scope.languageSelected.text).then(function(templateResponse) {
				if(typeof(templateResponse) != "undefined" || templateResponse != null || templateResponse != "") {
					// Call a method in syncManager which will retrieve questions based on this template id from question master.
					syncModuleFactory.downloadQuestionnaireData(templateResponse).then(function(questionnaireresponse) {
						if(questionnaireresponse == dealerAudit_ConstantsConst.success) {
							console.log("Questionnaire downloaded successfully");
						} else {
							console.log("Questionnaire ------------- not ---------- downloaded");
						}
					}, function(error) {
						console.log("Error downloading questionnaire based on template settingsController " + JSON.stringify(error));
					})
				}
			})
